        ...
        <div>
          ...
          <div className="mt-6 flex flex-wrap gap-x-6 gap-y-2 text-xs text-muted-foreground">
            <span>
              ...
              {t("dashboard.tools-count", "tools")}
            </span>
            <span>
              <strong className="mr-1 text-foreground">{categoryCount}</strong>
              {t("dashboard.categories-count", "categories")}
            </span>
            <span className="inline-flex items-center gap-1 text-primary">
              {t("dashboard.local-first", "Runs locally")} <ArrowUpRight className="size-3.5" />
            </span>
          </div>
        </div>

        <label className="relative block" htmlFor="dashboard-search">
          <span className="sr-only">{t("search.placeholder", "Search tools")}</span>
