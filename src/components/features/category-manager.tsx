    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 items-start sm:items-center justify-between">
        ...
        <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
          {/* 排序选择 */}
          <Select value={sortOption.key} onValueChange={handleSortChange}>
...
function SortableCategoryCard({
  ...
  return (
    <Card ref={setNodeRef} style={style} className={`${isDragging ? 'shadow-lg opacity-50' : ''}`}>
          ...
          {category.tools.length > 0 && (
            <div className="flex flex-wrap gap-1">
              {category.tools.map((toolSlug: string) => {
                ...
                return tool ? (
                  <Badge key={toolSlug} variant="outline" className="text-xs">
                    {tool.name}
                    <button className="ml-1 hover:text-destructive" onClick={() => onRemoveTool(category.id, toolSlug)}>
                      ×
                    </button>
                  </Badge>
                ) : null
