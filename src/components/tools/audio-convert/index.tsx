...
// 导入工具函数
import { generateId, downloadFile, createZipFromFiles, formatFileSize, downloadFromUrl } from '@/lib/file-utils'
...
import { audioFormats, audioTemplates, useAudioAnalysis, useAudioConversion, validateAudioFile } from './hooks'
...
const AudioConvert = () => {
  ...
  const { convertAudio, isProcessing, progress } = useAudioConversion()
  ...
  const handleBatchConvert = useCallback(async () => {
    ...
    try {
      for (const audio of pendingAudios) {
        ...
        try {
          const result = await convertAudio(audio.file, convertSettings)
          const processingTime = Date.now() - (audio.processingTime || Date.now())
          ...
          setAudios(prev => prev.map(a =>
            a.id === audio.id
              ? {
                  ...
                  processingTime,
                  targetFormat: convertSettings.format,
                  compressionRatio: audio.size / result.size,
                  qualityScore: calculateQualityScore(audio, result)
                }
              : a
          ))

          toast.success(`${audio.name} converted successfully`)
        } catch (error) {
          // 更新状态为错误
          setAudios(prev => prev.map(a =>
            a.id === audio.id
              ? { ...a, status: 'error', error: error instanceof Error ? error.message : 'Conversion failed' }
              : a
          ))

          toast.error(`Failed to convert ${audio.name}`)
        }
      }
  ...
  }, [audios, convertSettings, convertAudio, addToHistory])

  // 导出处理
  const handleExportSingle = useCallback((audio: AudioFile) => {
    ...
    downloadFromUrl(audio.convertedUrl, filename)
  ...
  const handleExportAll = useCallback(async () => {
    ...
    try {
      const files = convertedAudios.map(audio => ({
        name: `${audio.name.replace(/\.[^/.]+$/, '')}_converted.${convertSettings.format}`,
        content: fetch(audio.convertedUrl!).then(r => r.arrayBuffer())
      }))

      const zipBlob = await createZipFromFiles(files)
      downloadFile(zipBlob, 'converted_audios.zip')
        ...
        <div id="main-content" className="flex flex-col gap-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Convert audio files between different formats with advanced settings, batch processing, and quality analysis.
            ...
            <TabsContent value="templates">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Pre-configured settings for common audio conversion scenarios
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <TabsContent value="analysis">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Statistics and insights about your audio conversions
                  </CardDescription>
                </CardHeader>
                <CardContent>
            ...
            <TabsContent value="history">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Previous conversion sessions and settings
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  {history.length === 0 ? (
                            ...
                            <div className="flex items-start justify-between">
                              <div className="flex-1">
                                ...
                                <div className="text-sm">
                                  Format: {entry.settings.format.toUpperCase()} •
                                  Bitrate: {entry.settings.bitrate}kbps •
            ...
            <TabsContent value="help">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Learn about audio formats and conversion settings
                  </CardDescription>
                </CardHeader>
                <CardContent>
