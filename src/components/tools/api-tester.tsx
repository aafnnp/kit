...
export function ApiTester() {
  ...
  const updateHeader = useCallback((id: string, field: keyof ApiHeader, value: string | boolean) => {
    setState(prev => ({
      ...
      currentRequest: {
        ...prev.currentRequest,
        headers: prev.currentRequest.headers.map(h => 
  ...
  const updateParam = useCallback((id: string, field: keyof ApiParam, value: string | boolean) => {
    setState(prev => ({
      ...
      currentRequest: {
        ...prev.currentRequest,
        params: prev.currentRequest.params.map(p => 
  ...
  // 复制响应
  const copyResponse = useCallback((result: ApiTestResult) => {
    if (result.response) {
  ...
  const getStatusColor = (status: number) => {
    ...
    switch (category) {
      ...
      case 'client-error': return 'bg-yellow-100 text-yellow-800'
      case 'server-error': return 'bg-red-100 text-red-800'
  ...
  return (
    <ToolBase
      ...
      description={t('tools.api-tester.description', 'Test REST APIs with customizable requests and detailed responses')}
    >
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex gap-2">
              ...
              >
                ...
                {t('tools.api-tester.params', 'Parameters')} ({state.currentRequest.params.filter(p => p.enabled && p.key).length})
              </Button>
              ...
              >
                ...
                {t('tools.api-tester.headers', 'Headers')} ({state.currentRequest.headers.filter(h => h.enabled && h.key).length})
              </Button>
            </div>
