...
import type { Matrix, MatrixOperation, OperationType } from '@/types/matrix-math'
        ...
        onComplete: (workerResult) => {
          ...
          const matrixOperation: MatrixOperation = {
            ...
            analysis: {
              ...
              stability: calculateNumericalStability(matrices),
...
export function useMatrixAnalysis() {
  const analyzeMatrix = useCallback((matrix: Matrix) => {
    ...
    if (isSquare) {
      // 对称性检查
      isSymmetric = data.every((row, i) => 
        row.every((val, j) => Math.abs(val - data[j][i]) < 1e-10)
      )
      
      // 单位矩阵检查
      isIdentity = data.every((row, i) => 
        row.every((val, j) => Math.abs(val - (i === j ? 1 : 0)) < 1e-10)
      )
      
      // 对角矩阵检查
      isDiagonal = data.every((row, i) => 
        row.every((val, j) => i === j || Math.abs(val) < 1e-10)
      )
      
      // 上三角矩阵检查
      isUpperTriangular = data.every((row, i) => 
        row.every((val, j) => j >= i || Math.abs(val) < 1e-10)
      )
      
      // 下三角矩阵检查
      isLowerTriangular = data.every((row, i) => 
        row.every((val, j) => j <= i || Math.abs(val) < 1e-10)
      )
    ...
    const nonZeroElements = data.reduce((count, row) => 
      count + row.reduce((rowCount, val) => 
        rowCount + (Math.abs(val) > 1e-10 ? 1 : 0), 0
      ), 0
    )
...
function calculateNumericalStability(matrices: Matrix[]): number {
  ...
  for (const matrix of matrices) {
    ...
    const minElement = Math.min(...data.flat().filter(x => Math.abs(x) > 1e-10).map(Math.abs))
...
export {
  generateId,
  calculateNumericalStability,
  calculateMemoryUsage,
  getExpectedMatrixCount
}