...
const useSlugExport = () => {
  ...
  const exportAllSlugs = useCallback((slugs: SlugResult[], filename: string) => {
    const content = slugs.map((slug) => `${slug.original} -> ${slug.slug}`).join('\n')
...
const SlugGeneratorCore = () => {
  ...
  const stats: SlugStats = useMemo(() => {
    ...
    return {
      ...
      averageLength: allSlugs.length > 0 ? allSlugs.reduce((sum, s) => sum + s.length, 0) / allSlugs.length : 0,
      averageSeoScore: allSlugs.length > 0 ? allSlugs.reduce((sum, s) => sum + s.seoScore, 0) / allSlugs.length : 0,
      ...
      <div id="main-content" className="flex flex-col gap-4">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Generate SEO-friendly URL slugs from text with customizable options. Supports batch processing, multiple
              file formats, and real-time generation with SEO scoring. Use keyboard navigation: Tab to move between
              ...
              {manualText.trim() && (
                <div className="space-y-4">
                  <div className="p-4 bg-muted/30 rounded-lg">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex gap-2">
                        ...
                        >
                          {copiedText === 'slug' ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}
                        </Button>
                        <Button
                          size="sm"
                          variant="outline"
                    ...
                    <div className="space-y-2">
                      <div className="p-3 bg-background rounded border">
                        ...
                        <div className="font-mono text-sm break-all">
                          {manualSlugResult.slug || (
                            <span className="text-muted-foreground">No valid slug generated</span>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                        ...
                        <div className="text-center p-2 bg-background rounded border">
                          <div
                            className={`text-lg font-bold ${manualSlugResult.isValid ? 'text-green-600' : 'text-red-600'}`}
                          >
                  ...
                  <div className="flex items-center justify-between">
                    <Button variant="outline" onClick={() => setShowBatchResults(!showBatchResults)}>
                      <RefreshCw className="h-4 w-4 mr-2" />
                      {showBatchResults ? 'Hide' : 'Show'} Line-by-Line Results
                    </Button>

                    {showBatchResults && batchSlugResults.length > 0 && (
                      <Button variant="outline" onClick={() => exportAllSlugs(batchSlugResults, 'manual_text')}>
                          ...
                          <div key={index} className="p-3 bg-background rounded border">
                            <div className="flex items-center justify-between mb-1">
                              ...
                              <div className="flex items-center gap-2">
                                <span
                                  className={`text-xs px-2 py-1 rounded ${result.isValid ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' : 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400'}`}
                                >
                    ...
                    <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-950/20 rounded-lg">
                      <div className="text-center">
                        <span className="text-blue-700 dark:text-blue-400 font-semibold">
                          Valid slugs: {stats.validSlugs} / {stats.totalSlugs} (
                          {((stats.validSlugs / stats.totalSlugs) * 100).toFixed(1)}%)
                        </span>
                      </div>
                    </div>
                  )}
                                  ...
                                  <div className="max-h-40 overflow-y-auto space-y-1">
                                    {file.slugs.slice(0, 5).map((slug, index) => (
                                      <div key={index} className="p-2 bg-muted/30 rounded text-xs">
                                        <div className="flex items-center justify-between mb-1">
                                          <span
                                            className={`px-2 py-1 rounded text-xs ${slug.isValid ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' : 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400'}`}
                                          >
                                            SEO: {slug.seoScore}
                                          </span>
                                          <Button
