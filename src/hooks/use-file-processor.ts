...
export function useFileProcessor(config: FileProcessorConfig = {}) {
  ...
  // 读取文件内容
  const readFileContent = useCallback((file: File): Promise<string | ArrayBuffer> => {
  ...
  const calculateStats = useCallback((): FileStats => {
    ...
    if (files.length > 0) {
      largestFile = files.reduce((largest, current) => 
        current.size > largest.size ? current : largest
      )
      smallestFile = files.reduce((smallest, current) => 
