...
export function useInputHandler(config: InputHandlerConfig = {}) {
  ...
  // 格式化输入
  const formatInput = useCallback((inputValue: string) => {
  ...
  // 解析输入
  const parseInput = useCallback((inputValue: string) => {
  ...
  // 更新值
  const updateValue = useCallback((newValue: string, skipValidation = false) => {
  ...
  // 添加到历史
  const addToHistory = useCallback((inputValue: string, label?: string) => {
  ...
  // 从历史选择
  const selectFromHistory = useCallback((historyItem: InputHistoryItem) => {
...
// 专用的数字输入处理 Hook
export function useNumberInputHandler(config: InputHandlerConfig & {
  min?: number
  max?: number
  step?: number
  precision?: number
  ...
  const numberFormatter = useCallback((value: string) => {
