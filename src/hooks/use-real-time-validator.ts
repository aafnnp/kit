...
export function useRealTimeValidator(config: ValidationConfig) {
  ...
  // XML 验证
  const validateXML = useCallback((value: string): ValidationResult => {
    const errors: ValidationError[] = []
    const warnings: ValidationWarning[] = []
    const suggestions: ValidationSuggestion[] = []
  ...
  // YAML 验证（简化版）
  const validateYAML = useCallback((value: string): ValidationResult => {
    const errors: ValidationError[] = []
    const warnings: ValidationWarning[] = []
    const suggestions: ValidationSuggestion[] = []
  ...
  // CSV 验证
  const validateCSV = useCallback((value: string): ValidationResult => {
    const errors: ValidationError[] = []
    const warnings: ValidationWarning[] = []
    const suggestions: ValidationSuggestion[] = []
  ...
  // 正则表达式验证
  const validateRegex = useCallback((value: string): ValidationResult => {
    const errors: ValidationError[] = []
    const warnings: ValidationWarning[] = []
    const suggestions: ValidationSuggestion[] = []
  ...
  const validate = useCallback((value: string): ValidationResult => {
    ...
    switch (type) {
      ...
      case 'regex':
        ...
        break
      case 'custom':
        result = customValidator ? customValidator(value) : { isValid: true, errors: [], warnings: [], suggestions: [] }
        break
      default:
    ...
    return result
  }, [type, validateJSON, validateXML, validateYAML, validateCSV, validateEmail, validateURL, validateRegex, customValidator])
  ...
  // 立即验证
  const validateNow = useCallback((value: string) => {
    if (debounceRef.current) {
      clearTimeout(debounceRef.current)
    }
