...
export function useExportManager() {
  ...
  const convertToText = useCallback((data: any, options: ExportOptions) => {
    ...
    if (Array.isArray(data)) {
      return data.map((item) => (typeof item === 'object' ? JSON.stringify(item) : String(item))).join(lineEnding)
    }
  ...
  // XML 转换
  const convertToXML = useCallback((data: any) => {
    ...
    const objectToXML = (obj: any, rootName = 'root'): string => {
      ...
      if (Array.isArray(obj)) {
        return `<${rootName}>${obj.map((item, index) => objectToXML(item, `item_${index}`)).join('')}</${rootName}>`
      }
  ...
  // Markdown 转换
  const convertToMarkdown = useCallback((data: any) => {
    if (Array.isArray(data) && data.length > 0 && typeof data[0] === 'object') {
      // 表格格式
      const headers = Object.keys(data[0])
