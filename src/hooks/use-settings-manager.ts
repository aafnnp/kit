...
export function useSettingsManager<T extends Record<string, any>>(
  ...
  // 保存设置到本地存储
  const saveSettings = useCallback(async (settingsToSave: T) => {
  ...
  // 验证设置
  const validateSettings = useCallback((settingsToValidate: T): ValidationResult => {
    if (!enableValidation) {
  ...
  }, [settings, validateSettings, debouncedSave])
  ...
  // 获取字段定义
  const getFieldDefinition = useCallback((key: string): SettingField | undefined => {
  ...
  // 获取可见字段（考虑依赖关系）
  const getVisibleFields = useCallback((group: SettingGroup): SettingField[] => {
  ...
  // 检查字段是否有错误
  const hasFieldError = useCallback((key: string): boolean => {
